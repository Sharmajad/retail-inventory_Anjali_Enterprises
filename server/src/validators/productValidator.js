...
const productValidator = [
  ...
  body('costPrice').optional({ nullable: true, checkFalsy: true }).isFloat({ min: 0 }).withMessage('Cost price must be a non-negative number'),
  body('sellingPrice').optional({ nullable: true, checkFalsy: true }).isFloat({ min: 0 }).withMessage('Selling price must be a non-negative number'),
